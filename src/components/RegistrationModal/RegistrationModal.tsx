...
export const RegistrationModal = ({onClose, onRegister}: Props) => {
  ...
  return (
    <Modal title={'Регистрация'} onClose={onClose}>
      <Form>
        {/*<Row>
        ...
        <Row>
          ...
          {state.email.error && <Label htmlFor="reg-email" error >{state.email.error}</Label>}
          {state.email.warning && <Label htmlFor="reg-email" warning >{state.email.warning}</Label>}
        </Row>
        <Row>
          <Label htmlFor="reg-password1">Пароль</Label>
          <Input id="reg-password1" type={'password'} value={state.password1.value} onChange={getFieldSetter('password1')}/>
          {state.password1.error && <Label htmlFor="reg-email" error >{state.password1.error}</Label>}
          {state.password1.warning && <Label htmlFor="reg-email" warning >{state.password1.warning}</Label>}
        </Row>
        <Row>
          <Label htmlFor="reg-password2">Пароль</Label>
          <Input id="reg-password2" type={'password'} value={state.password2.value} onChange={getFieldSetter('password2')}/>
          {state.password2.error && <Label htmlFor="reg-email" error >{state.password2.error}</Label>}
          {state.password2.warning && <Label htmlFor="reg-email" warning >{state.password2.warning}</Label>}
        </Row>
        <Row>
          <Button wide large onClick={handleRegisterClick}>Зарегистрироваться</Button>
        </Row>
      </Form>
    </Modal>
  )
